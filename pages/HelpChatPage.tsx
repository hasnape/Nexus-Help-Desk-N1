  ...
  const handleSendMessage = async (e: React.FormEvent | null, messageTextOverride?: string) => {
    ...
    try {
      ...
      const aiResponse = await getFollowUpHelpResponse(
        ...
        'ticketCategory.GeneralQuestion',        // catégorie générique
        currentChat,
        1,                                      // N1
      ...
      const errorMsg: ChatMessage = {
        id: crypto.randomUUID(),
        sender: 'ai',
        text: t("appContext.error.aiFollowUpFailed", { error: error.message || 'Unknown'}),
        timestamp: new Date()
      };
      setChatHistory(prev => [...prev, errorMsg]);
          ...
          <div className="absolute top-1/2 -translate-y-1/2 end-4">
            ...
            >
              {isAutoReadEnabled ? (
                ...
                <SpeakerOffIcon className="w-5 h-5 text-slate-400" />
              )}
            </Button>
          </div>
        )}
          ...
          <div className="flex justify-start mb-4">
            <div className="max-w-xl lg:max-w-2xl px-4 py-3 rounded-xl shadow bg-slate-200 text-slate-800 rounded-bl-none">
              <div className="flex items-center">
                <p className="font-semibold text-sm me-2">
                  {t('ticketDetail.aiAssistantName')}
                </p>
      ...
      <div className="p-4 sm:p-6 bg-slate-100 border-t border-slate-200">
        {showCreateTicketButton && (
          <div className="text-center mb-4 p-3 bg-primary/10 rounded-md">
            <p className="text-sm text-primary-dark mb-2">
              {t('helpChat.createTicketExplanation')}
            </p>
        ...
        >
          ...
          <Button
            ...
            disabled={isLoadingAi || !newMessage.trim() || isListening}
