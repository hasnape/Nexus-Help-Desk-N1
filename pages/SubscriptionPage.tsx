...
const SubscriptionPage: React.FC = () => {
    ...
    const backLinkDestination = user?.role === UserRole.AGENT ? "/agent/dashboard" : user?.role === UserRole.MANAGER ? "/manager/dashboard" : "/dashboard";
    ...
    return (
        <div className="max-w-5xl mx-auto">
            ...
            <div className="text-center mb-12">
                <h1 className="text-4xl font-bold text-slate-800">{t('subscription.pageTitle', { default: "Manage Your Subscription" })}</h1>
                <p className="mt-4 text-lg text-slate-600">{t('subscription.pageSubtitle', { default: "Choose the plan that's right for your team." })}</p>
            </div>

            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
                <h2 className="text-xl font-semibold text-slate-700">{t('subscription.currentPlan.title', { default: "Your Current Plan" })}</h2>
                <div className="flex items-center gap-4 mt-2">
                    <PlanIcon planNameKey={currentUserPlanKey} />
                    <div className="text-start">
                        <p className="text-3xl font-bold text-primary">{t(currentUserPlanKey)}</p>
                        {currentUserPlan === 'Freemium' && <p className="text-slate-500 mt-1">{t('subscription.currentPlan.freemiumDesc', { default: 'You are currently on the free plan.' })}</p>}
                    </div>
                </div>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                {plans.map((plan) => (
                    <div key={t(plan.nameKey)} className={`border rounded-xl p-8 flex flex-col shadow-lg text-center ${currentUserPlan === t(plan.nameKey, {default: plan.nameKey}) ? 'bg-slate-50' : 'bg-white'}`}>
                        <div className="mx-auto mb-4">
                            <PlanIcon planNameKey={plan.nameKey} />
                        </div>
                        <h3 className="text-2xl font-bold text-slate-800">{t(plan.nameKey)}</h3>
                        ...
                        <div className="mt-8">
                            {t(currentUserPlanKey) === t(plan.nameKey) ? (
                                <div className="text-center py-3 px-4 bg-green-100 text-green-700 font-semibold rounded-md">
                                    {t('subscription.currentPlan.label', { default: 'Current Plan' })}
                                </div>
                            ) : (
            ...
            <div className="text-center mt-8 text-xs text-slate-500">
                <p>{t('subscription.disclaimer.title', { default: 'Subscriptions are managed by PayPal.' })}</p>
                <p>{t('subscription.disclaimer.text', { default: 'You can cancel anytime from your PayPal account.' })}</p>
            </div>
        </div>
    );
};
