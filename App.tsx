...
interface AppContextType {
  ...
  logout: () => void;
  signUp: (email: string, fullName: string, password: string, options: { lang: AppLocale; role: UserRole; companyName: string; secretCode?: string; }) => Promise<string | true>;
  tickets: Ticket[];
  addTicket: (ticketData: Omit<Ticket, 'id' | 'created_at' | 'updated_at' | 'user_id' | 'assigned_agent_id' | 'internal_notes' | 'current_appointment' | 'assigned_ai_level' | 'chat_history'>, initialChatHistory: ChatMessage[]) => Promise<Ticket | null>;
...
const signUp = async (
    ...
    try {
        if (role === UserRole.MANAGER) {
            ...
            if (findCompanyError) {
                console.error('Error querying for company:', JSON.stringify(findCompanyError, null, 2));
  ...
  const proposeOrUpdateAppointment = async (ticketId: string, details: Omit<AppointmentDetails, 'proposedBy' | 'id' | 'history'>, proposedBy: 'agent' | 'user', newStatus: AppointmentDetails['status']): Promise<void> => {
    ...
    else if (newStatus === 'rescheduled_by_user') chatMessageText = translateHook('appointment.chat.userWantsToReschedule', { date: apptDateStr, time: proposedTime, location: locationOrMethod });
  ...
  const updateCompanyName = async (newName: string): Promise<boolean> => {
    if (!user || user.role !== UserRole.MANAGER || !user.company_id) {
        return false;
    }
    ...
    if (findError) {
        console.error("Error finding company to update:", JSON.stringify(findError, null, 2));
  ...
  return (
    <AppContext.Provider value={{ 
        user, login, logout, signUp, tickets, addTicket, updateTicketStatus, addChatMessage, sendAgentMessage,
