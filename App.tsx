...
const AppProviderContent: React.FC<{ children: ReactNode }> = ({
    ...
    try {
      if (role === UserRole.MANAGER) {
        if (secretCode !== "123456") {
          throw new Error(
            translateHook("signup.error.invalidSecretCodeManager")
          );
        }
        ...
        const { data: existingCompany, error: findCompanyError } =
          await supabase
            .from("companies")
            .select("id")
            .eq("name", companyName)
      ...
      // 📧 Envoi de l'email de bienvenue pour les managers
      if (role === UserRole.MANAGER) {
        try {
          const emailData = {
  ...
  const proposeOrUpdateAppointment = async (
    ...
    if (newStatus === "pending_user_approval")
      ...
      chatMessageText = translateHook(
        "appointment.chat.userWantsToReschedule",
  ...
  const updateCompanyName = async (newName: string): Promise<boolean> => {
    ...
    if (findError) {
      console.error(
        "Error finding company to update:",
    ...
    if (!companyResults || companyResults.length === 0) {
      console.error(
        "Data integrity error: company not found with name",
        oldName
      );
      alert(
        translateHook("managerDashboard.companyInfo.updateError", {
