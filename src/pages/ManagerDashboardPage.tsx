...
const isAbortFetchError = (error: unknown): boolean => {
  ...
  if (typeof error === 'string') return error.toLowerCase().includes('abort');
  const anyError = error as { name?: unknown; message?: unknown };
  if (typeof anyError.name === 'string' && anyError.name.toLowerCase() === 'aborterror') return true;
...
const normalizePlanKey = (plan: string | null | undefined): string | null => {
  ...
  if (normalized.includes('unlimit') || normalized.includes('illimit') || normalized.includes('enterprise')) return 'unlimited';
  if (normalized.includes('pro')) return 'pro';
  if (normalized.includes('standard') || normalized.includes('team')) return 'standard';
  if (normalized.includes('free') || normalized.includes('freemium') || normalized.includes('trial') || normalized.includes('sandbox')) return 'freemium';
...
const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode }> = ({ title, value, icon }) => (
  <div className="bg-surface p-4 rounded-lg shadow-md flex items-center">
    <div className="bg-primary/10 text-primary p-3 rounded-full me-4">{icon}</div>
    <div>
...
const ManagerTicketRow: React.FC<ManagerTicketRowProps> = ({ ticket, agents, allUsers, onAssignTicket, onDeleteTicket }) => {
  ...
  const agentOptions = agents.map((agent) => ({ value: agent.id, label: agent.full_name }));
  const handleAssign = () => onAssignTicket(ticket.id, selectedAgent || null);
...
const UserManagementRow: React.FC<UserManagementRowProps> = ({ userToManage, currentUser, onUpdateRole, onDeleteUser }) => {
  ...
  const handleDeleteUser = () => {
    ...
    if (confirmation) onDeleteUser(userToManage.id);
  };
  ...
  return (
    <tr className="hover:bg-slate-50">
      <td className="p-3 font-medium text-slate-800">
        {userToManage.full_name} <span className="text-slate-500 text-xs">({userToManage.email})</span>
      </td>
      <td className="p-3 text-slate-600">{languageNames[(userToManage.language_preference as Locale) || 'en']}</td>
      <td className="p-3">
        <div className="flex items-center space-x-2 rtl:space-x-reverse">
          {!isProtectedUser ? (
            <div className="flex items-center space-x-2 rtl:space-x-reverse">
...
const ManagerDashboardPage: React.FC = () => {
    ...
    const fetchCompanyDetails = async () => {
      if (user?.role === UserRole.MANAGER && user.company_id) {
        try {
          ...
          if (error) {
            if (!isAbortFetchError(error)) console.error('Error fetching company details:', JSON.stringify(error, null, 2));
          ...
          if (!isAbortFetchError(e)) console.error('Critical error fetching company details:', JSON.stringify(e, null, 2));
        }
      }
    };
    fetchCompanyDetails();
  ...
  useEffect(() => {
    if (!user?.company_id || user.role !== UserRole.MANAGER) {
      setQuotaState({ loading: false, error: false, limit: null, isUnlimited: false, planKey: null, rawPlanLabel: null, timezone: DEFAULT_TIME_ZONE });
      setRpcUsed(null);
      return;
    }
    let isMounted = true;
    ...
    setQuotaState((previous) => ({ ...previous, loading: true, error: false }));
    setRpcUsed(null);
    ...
    resolveCompanyQuota(user.company_id, controller.signal)
      ...
      .catch((error) => {
        if (!isMounted) return;
        if (isAbortFetchError(error)) return;
        console.error('[quota] Failed to load quota metadata', error);
        setRpcUsed(null);
        setQuotaState((previous) => ({ ...previous, loading: false, error: true }));
      });
  ...
  const ticketsUsedThisMonth = useMemo(() => {
    ...
    return tickets.reduce((count, ticketItem) => {
      ...
      if (isDateInCurrentMonth(createdAt, tz)) return count + 1;
      return count;
    }, 0);
  ...
  const quotaSeverity = useMemo<QuotaSeverity>(() => {
    if (normalizedQuota.unlimited || normalizedQuota.limit === null || normalizedQuota.percent === null) return 'none';
    if (normalizedQuota.percent >= 100) return 'blocked';
    if (normalizedQuota.percent >= 80) return 'near';
    return 'normal';
  }, [normalizedQuota]);
  ...
  const quotaStyles = useMemo(() => {
    switch (quotaSeverity) {
      case 'blocked':
        return { container: 'bg-red-50 border-red-300', message: 'text-red-700' };
      case 'near':
        return { container: 'bg-amber-50 border-amber-300', message: 'text-amber-700' };
      case 'normal':
        return { container: 'bg-surface border-slate-200', message: 'text-slate-600' };
      case 'none':
      default:
        return { container: 'bg-surface border-slate-200', message: 'text-slate-600' };
    }
  ...
  const { quotaDisplayLabel, quotaAriaLabel } = useMemo(() => {
    ...
    const quotaDisplay = t('dashboard.quota.remaining', {
      ...
      values: { remaining: normalizedQuota.remainingLabel, limit: normalizedQuota.limitLabel, percentChunk },
    ...
    const quotaAria = t('dashboard.quota.remaining', {
      default: '{{remaining}} / {{limit}}{{percentChunk}}',
      values: { remaining: normalizedQuota.remainingLabel, limit: ariaLimit, percentChunk },
      remaining: normalizedQuota.remainingLabel,
      limit: ariaLimit,
      percentChunk,
    });
  ...
  const quotaMessage = useMemo(() => {
    if (quotaState.loading || normalizedQuota.unlimited || normalizedQuota.limit === null) return null;
    if (quotaSeverity === 'near') return t('dashboard.quota.near_limit');
    if (quotaSeverity === 'blocked') return t('dashboard.quota.blocked');
    return null;
  ...
  const handleDeleteTicket = (ticketId: string) => {
    ...
    if (confirmation) deleteTicket(ticketId);
  };
  ...
  const statusOptionsForFilter = [
    { value: 'All', label: t('managerDashboard.filters.all', { default: 'All' }) },
    ...Object.values(TicketStatus).map((s) => ({ value: s as string, label: t(`ticketStatus.${s as string}`) })),
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="pb-4 border-b border-slate-300">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
          <div className="mb-4 sm:mb-0">
            <h1 className="text-3xl font-bold text-textPrimary">{t('managerDashboard.title', { username: user.full_name })}</h1>
      ...
      <section className={`border shadow-lg rounded-lg p-4 sm:p-6 transition-colors ${quotaStyles.container}`} aria-live="polite">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-slate-600">{t('dashboard.quota.title')}</p>
            <p className="text-2xl font-bold text-slate-800 mt-1" aria-label={quotaAriaLabel}>
              {quotaDisplayLabel}
            </p>
            {quotaMessage && <p className={`mt-2 text-sm ${quotaStyles.message}`}>{quotaMessage}</p>}
          </div>
        ...
        <div className="bg-slate-100 p-4 rounded-md space-y-4">
            ...
            <div>
              ...
              {!isEditingName ? (
                ...
                <div className="flex items-center gap-2">
                  <Input value={newCompanyName} onChange={(e) => setNewCompanyName(e.target.value)} className="flex-grow" disabled={isUpdatingName} />
          ...
          <div>
            <div className="mt-4 bg-slate-200 p-3 rounded text-sm">
              <p className="text-slate-600 mb-2">{t('managerDashboard.companyInfo.onboardingInstructions')}</p>
              <div className="flex items-center gap-3">
                <p className="text-base font-mono bg-slate-300 text-slate-800 p-2 rounded select-all flex-grow text-center">{company?.name || '...'}</p>
                <Button
                  variant="outline"
                  size="md"
      ...
      <section className="bg-surface shadow-lg rounded-lg p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 lg:flex lg:items-end lg:gap-2">
            ...
            <Button variant="outline" onClick={handleResetFilters} className="w-full lg:w-auto mt-4 sm:mt-0">
