...
const GuideOnboardingPage: React.FC = () => {
  ...
  return (
    <div className="bg-slate-950 text-white min-h-screen">
      <header className="bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950">
        <div className="max-w-6xl mx-auto px-4 py-20 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4" role="group" aria-label={t("guideOnboarding.hero.ctaGroupLabel", { defaultValue: "Actions principales" }) || undefined}>
            <Link
              to="/demo"
              className="inline-flex items-center justify-center px-6 py-3 rounded-full bg-primary text-slate-950 font-semibold"
            >
              {t("guideOnboarding.hero.ctaDemo", { defaultValue: "Demander une démo" })}
            </Link>
            <Link
              to="/contact"
              className="inline-flex items-center justify-center px-6 py-3 rounded-full border border-white/30 text-white font-semibold"
            >
      ...
      <main>
        <section aria-labelledby="guide-pillars" className="py-16 px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {pillars.map((pillar) => (
                <article key={pillar.key} className="bg-slate-900/60 border border-white/5 rounded-3xl p-6 shadow-lg">
                  <p className="text-primary text-sm font-semibold uppercase tracking-wider">
                    {pillar.eyebrow}
                  </p>
                  <h3 className="text-2xl font-semibold mt-2 mb-3">{pillar.title}</h3>
                  <p className="text-slate-200 mb-4">{pillar.description}</p>
                  <ul className="space-y-2 text-slate-100 list-disc pl-5">
        ...
        <section aria-labelledby="guide-steps" className="py-16 px-4 bg-slate-900">
          <div className="max-w-5xl mx-auto">
            ...
            <ol className="space-y-6">
              {steps.map((step, index) => (
                <li key={index} className="bg-slate-800/70 border border-white/5 rounded-2xl p-5">
                  <div className="flex items-start gap-4">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/20 text-primary font-bold">
                      {index + 1}
                    </span>
        ...
        <section aria-labelledby="guide-accessibility" className="py-16 px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <ul className="space-y-4">
              {accessibilityChecks.map((item, index) => (
                <li key={index} className="flex items-start gap-4 bg-slate-900/60 border border-white/5 rounded-2xl p-5">
