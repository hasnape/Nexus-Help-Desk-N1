...
const TicketDetailPage: React.FC = () => {
  ...
  const handleNewAiMessageForAutoRead = (aiMessage: ChatMessageType) => {
    if (isProPlan && isAutoReadEnabled && browserSupportsTextToSpeech && aiMessage.id !== speakingMessageId && aiMessage.sender === 'ai') {
  ...
  const submitMessage = async () => {
    ...
    const messageToSubmit = newMessage.trim();
    if (messageToSubmit === '' || !ticketId || isLoadingAi) return;
    ...
    if (user.role === UserRole.AGENT || user.role === UserRole.MANAGER) {
      await sendAgentMessage(ticketId, messageToSubmit);
    } else {
      await addChatMessage(ticketId, messageToSubmit, handleNewAiMessageForAutoRead);
    }
  };
  ...
  const handleProposeAppointment = async () => {
    if (!isProPlan || !apptDate || !apptTime || !apptLocationMethod || !ticketId || (user.role !== UserRole.AGENT && user.role !== UserRole.MANAGER)) return;
  ...
  const handleUserAcceptAppointment = () => {
    if (!isProPlan || !ticket.current_appointment || !ticketId) return;
    proposeOrUpdateAppointment(
        ticketId,
  ...
  const canRepeatLastMessage = (lastSpokenAiMessage || ticket.chat_history.some(m => m.sender === 'ai')) && browserSupportsTextToSpeech && isProPlan;
  ...
  const renderCurrentAppointmentInfo = () => {
    if (!ticket.current_appointment || !isProPlan) return null;
  ...
  return (
    <div className="max-w-4xl mx-auto bg-surface shadow-xl rounded-lg overflow-hidden flex flex-col h-[calc(100vh-10rem)] sm:h-[calc(100vh-8rem)]">
      ...
      <footer className="p-4 sm:p-6 bg-slate-100 border-t border-slate-200">
        <form onSubmit={handleSendMessage} className="flex items-start space-x-2 sm:space-x-3 rtl:space-x-reverse">
          <Textarea
            value={newMessage}
            onChange={e => setNewMessage(e.target.value)}
            placeholder={isListeningChatInput ? t('ticketDetail.chatPlaceholder.listening') : (isAgentOrManager ? t('ticketDetail.chatPlaceholder.agent') : t('ticketDetail.chatPlaceholder.user'))}
            rows={2}
          ...
          />
          {browserSupportsSpeechRecognition && isProPlan && (
            <Button type="button" onClick={handleChatMicButtonClick} variant={isListeningChatInput ? 'danger' : 'secondary'} size="md" className="h-full px-3 sm:px-4 self-stretch !py-0" aria-label={isListeningChatInput ? t('ticketDetail.micButton.stopRecording') : t('ticketDetail.micButton.startRecording')} disabled={isLoadingAi || isTicketClosedOrResolved} title={isListeningChatInput ? t('ticketDetail.micButton.stopRecording') : t('ticketDetail.micButton.startRecording')}>
              <MicrophoneIcon className="w-5 h-5" />
            </Button>
          )}
          <Button type="submit" variant="primary" size="md" className="h-full px-3 sm:px-5 self-stretch !py-0" disabled={isLoadingAi || !newMessage.trim() || isListeningChatInput || isTicketClosedOrResolved} isLoading={isLoadingAi}>
