...
const SignUpPage: React.FC = () => {
  ...
  const pricingTiers = [
    {
        ...
        features: ["pricing.freemium.feature1", "pricing.freemium.feature2", "pricing.freemium.feature3"],
    },
    {
        ...
        features: ["pricing.standard.feature1", "pricing.standard.feature2", "pricing.standard.feature3", "pricing.standard.feature4"],
    },
    {
        ...
        features: ["pricing.pro.feature1", "pricing.pro.feature2", "pricing.pro.feature3", "pricing.pro.feature4"],
    }
  ];
  ...
  const validateDetails = (): boolean => {
    ...
    if (role === UserRole.MANAGER && plan !== 'freemium' && !activationCode.trim()) {
        setError(t('signup.error.activationCodeRequired', {default: "Activation code is required for Standard and Pro plans."}));
        return false;
    }
    setError('');
    return true;
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (role === UserRole.MANAGER) {
        if (plan === 'standard' && activationCode.trim() !== 'STANDARD_SECRET_CODE') {
            setError(t('signup.error.invalidActivationCode', { default: 'The activation code is not valid for the Standard plan.' }));
            return;
        }
        if (plan === 'pro' && activationCode.trim() !== 'PRO_SECRET_CODE') {
            setError(t('signup.error.invalidActivationCode', { default: 'The activation code is not valid for the Pro plan.' }));
            return;
        }
    }
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-700 p-4">
            <div className="bg-surface p-8 rounded-xl shadow-2xl w-full max-w-md text-center">
                <LoadingSpinner size="lg" text={t('signup.finalizing', {default: "Finalizing your account..."})} />
                <p className="mt-4 text-slate-500">{t('signup.finalizingSubtitle', {default: "Please wait, you will be redirected shortly."})}</p>
            </div>
        </div>
      );
  }
          ...
          <>
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Input label={t('signup.emailLabel')} id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
                <Input label={t('signup.fullNameLabel')} id="fullName" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
                <Input label={t('signup.passwordLabel')} id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required />
                <Input label={t('signup.confirmPasswordLabel')} id="confirmPassword" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
              </div>

              <Select label={t('signup.roleLabel')} id="role" value={role} onChange={(e) => setRole(e.target.value as UserRole)} options={roleOptions} required />
              
              <div>
                <Input label={role === UserRole.MANAGER ? t('signup.companyNameLabel') : t('signup.existingCompanyNameLabel')} id="companyName" type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder={role === UserRole.MANAGER ? t('signup.companyNamePlaceholder') : t('signup.existingCompanyNamePlaceholder')} required />
                <p className="mt-1 text-xs text-slate-500 px-1">{role === UserRole.MANAGER ? t('signup.companyNameHelp.manager') : t('signup.companyNameHelp.employee')}</p>
              </div>

              {role === UserRole.MANAGER && (
                  <div className="space-y-4 pt-4">
                      <label className="block text-sm font-medium text-slate-700 mb-2">{t('signup.planLabel', { default: "Subscription Plan" })}</label>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {pricingTiers.map((tier) => (
                          ...
                          >
                            ...
                            <h3 className="font-bold text-slate-800 text-lg">{t(tier.nameKey)}</h3>
                            <div className="my-2">
                                <span className="text-3xl font-bold">{t(tier.priceKey)}</span>
                                {tier.planValue !== 'freemium' && <span className="text-slate-500 ms-1 text-sm">{t('pricing.perAgentPerMonth')}</span>}
                            </div>
                      ...
                      {plan !== 'freemium' && (
                          <div className="mt-4 space-y-2 bg-slate-50 p-4 rounded-lg border border-slate-200">
                              <Input 
                                label={t('signup.activationCodeLabel', {default: "Activation Code"})}
                                id="activationCode"
                                value={activationCode}
                                onChange={e => setActivationCode(e.target.value)}
                                placeholder={t('signup.activationCodePlaceholder', {default: "Enter code from support"})}
                                required
                              />
                              <p className="text-xs text-slate-600">
                                {t('signup.activationCodeHelp.prefix', {default: "You need a code to sign up for a paid plan."})}
                                <a 
                                  href={`mailto:hubnexusinfo@gmail.com?subject=${encodeURIComponent(`Request for Activation Code - ${plan.charAt(0).toUpperCase() + plan.slice(1)} Plan`)}&body=${encodeURIComponent(`Hello,\n\nOur company, [YOUR COMPANY NAME HERE], would like to sign up for the ${plan} plan. Please provide us with an activation code.\n\nThank you.`)}`} 
                                  className="ms-1 text-primary hover:underline font-semibold"
                                >
                                  {t('signup.activationCodeHelp.link', {default: "Request one via email."})}
                                </a>
                              </p>
                          </div>
                      )}
              ...
              <Button type="submit" className="w-full !mt-6" size="lg" isLoading={isLoading} disabled={isLoading}>
                {t('signup.signUpButton')}
              </Button>
            </form>
             <div className="mt-6 text-center">
                <p className="text-sm text-slate-500">
                    {t('signup.alreadyHaveAccount')}{' '} <Link to="/login" className="font-medium text-primary hover:text-primary-dark">{t('signup.signInLink')}</Link>
                </p>
