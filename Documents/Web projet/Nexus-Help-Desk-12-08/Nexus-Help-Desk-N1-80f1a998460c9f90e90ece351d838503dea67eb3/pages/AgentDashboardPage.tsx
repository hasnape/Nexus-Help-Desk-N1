...
const AgentTicketRow: React.FC<{ ticket: Ticket, onTakeCharge?: (ticketId: string) => void, isUnassigned?: boolean }> = ({ ticket, onTakeCharge, isUnassigned }) => {
...
const AgentDashboardPage: React.FC = () => {
    ...
    const unassignedTickets = tickets.filter(ticket => !ticket.assigned_agent_id)
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    
    const myTickets = tickets.filter(ticket => ticket.assigned_agent_id === user.id)
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-100">
                                <tr>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.title')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.client')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.workstation')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.created')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.status')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.action')}</th>
                                </tr>
                            </thead>
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                             <thead className="bg-slate-100">
                                <tr>
                                    ...
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.client')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.workstation')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.lastUpdated')}</th>
                                     <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.status')}</th>
                                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('agentDashboard.tableHeader.action')}</th>
                                </tr>
                            </thead>
